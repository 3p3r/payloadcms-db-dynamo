...
describe('where operators (FilterExpression pushdown)', () => {
  ...
  it('contains on text (JS-evaluated)', async () => {
    const result = await handle.payload.find({
      collection: 'items',
      where: { description: { contains: 'root' } },
    })
    expect(result.totalDocs).toBe(1)
    expect(result.docs[0]?.name).toBe('carrot')
  })

  it('not_like (JS-evaluated)', async () => {
    const result = await handle.payload.find({
      collection: 'items',
      where: { description: { not_like: 'sweet' } },
    })
    expect(result.docs.map((d) => d.name).sort()).toEqual([
      'apple',
      'banana',
      'carrot',
      'eggplant',
...
describe('findDistinct', () => {
  ...
  it('paginates distinct values', async () => {
    const page1 = await handle.payload.findDistinct({
      collection: 'items',
      field: 'name',
      limit: 2,
      page: 1,
    ...
    const page2 = await handle.payload.findDistinct({
      collection: 'items',
      field: 'name',
      limit: 2,
      page: 2,
