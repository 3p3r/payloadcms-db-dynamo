...
function nestedFields(field: FlattenedField): Field[] {
  return 'fields' in field && Array.isArray(field.fields) ? field.fields : []
}

const NESTED_FIELD_PICKERS: Record<string, (value: unknown, field: FlattenedField) => unknown> = {
  array: (value, field) => pickArrayItems(value, nestedFields(field)),
  blocks: (value, field) =>
    pickBlockItems(value, 'blocks' in field && Array.isArray(field.blocks) ? field.blocks : []),
  group: (value, field) => pickAtLevel(value, nestedFields(field)),
  tab: (value, field) => pickAtLevel(value, nestedFields(field)),
}
...
function pickAtLevel(
  ...
  for (const field of flattenTopLevelFields(fields) as FlattenedField[]) {
    ...
    const nested = NESTED_FIELD_PICKERS[field.type]
    out[name] = nested ? nested(value, field) : value
  }
